...
import { AuthProvider, SubscriptionRepository } from '../../../application/ports';
...
export function createSubscriptionRouter({ authProvider, subscriptionRepository }: SubscriptionRouterOptions): Router {
  const router = Router();
  ...
  router.get('/', authenticate(authProvider), createSubscriptionsController(getSubscriptionsUseCase));
